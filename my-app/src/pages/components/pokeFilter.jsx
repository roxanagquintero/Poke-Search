...
export default function PokeFilter(props) {
  const { pokemonData, setFilterNames, setFilterNumbers, setFilterHabitats, setFilterGenerations } = props;
  ...
  const uniqueHabitats = useMemo(() => Array.from(new Set(pokemonData.map((pokemon) => pokemon.habitat.name))), [pokemonData]);
  const uniqueGenerations = useMemo(() => Array.from(new Set(pokemonData.map((pokemon) => pokemon.generation.name))), [pokemonData]);
  ...
  function DropDown(data) {
    ...
    const uniqueOptions = data.searchby === "habitat" ? uniqueHabitats : (data.searchby === "generation" ? uniqueGenerations : data.data);
      ...
      <Autocomplete
        ...
        onChange={(event, newValue) => {
          if (data.searchby === "name") {
            ...
            setFilterNames(newValue && newValue.map((item) => item.name) || []);
          } else if (data.searchby === "id") {
            setSelectedNumberOptions(newValue ? newValue : []);
            setFilterNumbers(newValue && newValue.map((item) => item.id.toString()) || []);
          } else if (data.searchby === "habitat") {
            setSelectedHabitatOptions(newValue ? newValue : []);
            setFilterHabitats(newValue && newValue.map((item) => item.name) || []);
          } else if (data.searchby === "generation") {
            setSelectedGenerationOptions(newValue ? newValue : []);
            setFilterGenerations(newValue && newValue.map((item) => item.name) || []);
          }
        }}
        value={
          data.searchby === "name" ? selectedNameOptions :
          (data.searchby === "id" ? selectedNumberOptions :
          (data.searchby === "habitat" ? selectedHabitatOptions :
          (data.searchby === "generation" ? selectedGenerationOptions : null)))
        }
    ...
    <>
      <div>
        ...
        <Autocomplete
          ...
          options={uniqueHabitats}  // Use unique habitats
        ...
        <Autocomplete
          ...
          options={uniqueGenerations}  // Use unique generations
          renderOption={(props, option) => (
            <li {...props} key={option}>
              {option}
            </li>
